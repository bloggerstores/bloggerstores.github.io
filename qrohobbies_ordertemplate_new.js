...
// ── Utilidad: leer parámetros GET de la URL ──
...
window.onload = function () {
  ...
  // ── Detectar si los datos vienen de params GET (modo orden genérica) ──
  var _fromParams = _getParam('cliente') !== '';

  var _0x3ece9f, _0xa24eca;

  if (_fromParams) {
    // ── MODO PARAMS GET: datos vienen de la URL ──
    _0x3ece9f = decodeURIComponent(_getParam('cliente'));
    var _dir    = decodeURIComponent(_getParam('dir')).replace(/\|/g, '<br>');
    var _tel    = decodeURIComponent(_getParam('tel'));
    var _email  = decodeURIComponent(_getParam('email'));
    var _entrega = decodeURIComponent(_getParam('entrega'));
    _0xa24eca  = (_tel    ? _tel + '<br>' : '')
               + (_email  ? _email + '<br>' : '')
               + (_dir    ? _dir + '<br>' : '')
               + (_entrega ? _entrega : '');
  } else {
    // ── MODO PLANTILLA: retrocompatible, lee el post-body ──
    _0x3ece9f = $(".post-body table .client_name").text().replace(/  |\r\n|\n|\r/gm, '');
    _0xa24eca = $(".post-body table .client_info").html();
  }
  ...
  if (_fromParams) {
    // ── MODO PARAMS GET: items en formato "Nombre|precio|qty,Nombre2|precio2|qty2" ──
    var _itemsRaw = decodeURIComponent(_getParam('items'));
    if (_itemsRaw) {
      _itemsRaw.split(',').forEach(function(item) {
        var parts = item.split('|');
        var _nombre = parts[0] ? parts[0].trim() : '';
        var _precio = parts[1] ? Number(parts[1].trim().replace(/\./g,'').replace(',','')) : 0;
        var _qty    = parts[2] ? Number(parts[2].trim()) : 1;
        var _subtotal = _precio * _qty;
        if (_nombre) {
          var row = '<tr>';
          row += "<td><b>" + _nombre + "</b><br/><small>$" + separator(_precio, $_config.text.currency, $_config.text.currency_direction) + " x " + _qty + "</small></td>";
          row += "<td>$" + separator(_subtotal, $_config.text.currency, $_config.text.currency_direction) + "</td>";
          row += '</tr>';
    ...
    // ── MODO PLANTILLA: lee filas de la tabla ──
  ...
  // ── Descuento ──
  var _0x43f2aa = _fromParams ? decodeURIComponent(_getParam('desc')) : $(".post-body table .discount_val").text().replace(/  |\r\n|\n|\r/gm, '');
  if (_0x43f2aa) {
    ...
    if (_0x43f2aa.includes('%')) {
      ...
      _0x452e32 = _0x5ab3e1 - Number(_0x43f2aa);
    }
  } else {
    _0x452e32 = _0x5ab3e1;
  }
  ...
  // ── IVA ──
  var _0x5ee149 = _fromParams ? decodeURIComponent(_getParam('iva')) : $(".post-body table .tax_val").text().replace(/  |\r\n|\n|\r/gm, '');
  if (_0x5ee149) {
    _0x5ee149 = _0x5ee149.toString().replaceAll('%', '');
    var $tax_count = _0x452e32 * Number(_0x5ee149) / 0x64;
  ...
  // ── Nota ──
  var _0x370c72 = _fromParams ? decodeURIComponent(_getParam('nota')) : $(".post-body table .note").text().replace(/  |\r\n|\n|\r/gm, '');
  if (_0x370c72) {
